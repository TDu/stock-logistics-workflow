...
class ProcurementGroup(models.Model):
    ...
    def _is_product_available(self, product_id, quantity):
        if quantity == 0:
            return False
        # FIXME with release channel may have to use mmediately_usable_qty ?
        return product_id.qty_available >= quantity
    ...
    def _get_rule(self, product_id, location_id, values):
        ...
        if sale_line_id:
            ...
            if sale_line and self._is_product_available(
                product_id, sale_line.product_uom_qty
            ):
