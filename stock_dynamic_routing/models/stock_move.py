...
class StockMove(models.Model):
    ...
    def _insert_routing_moves(self, picking_type, location, destination):
        ...
        # The routing rule that may apply to this new inserted move (reclassifying
        # its picking type and/or narrowing its destination) is applied by
        # _apply_routing_rule_pull, which appends the move to routing_to_apply.
        # Merging the move with identical existing moves is also handled there,
        # once the rule has been applied, so several orders share a single move.
